...
@RestController
@RequestMapping("/backend/practical-evaluation")
public class BackendPracticalEvaluationController {
    ...
    @Operation(summary = "实操评价打分（自动触发综合成绩汇总）")
    @PutMapping("/score")
    public Result<Map<String, Object>> score(@RequestBody @Validated ScoreRequest request) {
        ...
        if (!isSuperAdmin && !currentAdminId.equals(course.getTeacherId())) {
            return Result.fail("无权限对该课程实操评分");
        }
        ...
        List<PracticalEvaluation> evaluationList = practicalEvaluationService.list(new LambdaQueryWrapper<PracticalEvaluation>()
                .eq(PracticalEvaluation::getUserId, request.getUserId())
                .eq(PracticalEvaluation::getCourseId, request.getCourseId())
                .orderByDesc(PracticalEvaluation::getId));
        PracticalEvaluation evaluation = evaluationList.isEmpty() ? null : evaluationList.get(0);
        boolean isNew = evaluation == null;
        if (isNew) {
            evaluation = new PracticalEvaluation();
        ...
        boolean success = isNew ? practicalEvaluationService.save(evaluation) : practicalEvaluationService.updateById(evaluation);
        if (!success) {
            return Result.fail("实操评分保存失败");
        }
